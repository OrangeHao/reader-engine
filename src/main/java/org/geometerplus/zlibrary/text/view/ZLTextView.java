...
public abstract class ZLTextView extends ZLTextViewBase
{
	...
	private synchronized void gotoMark(ZLTextMark mark)
	{
		...
		if (myCurrentPage.StartCursor.isNull()) { return; }
	...
	{
		if (cursor == ZLTextSelectionCursor.None) { return null; }

		if (cursor == mySelection.getCursorInMovement()) { return mySelection.getCursorInMovementPoint(); }

		if (cursor == ZLTextSelectionCursor.Left)
		{
			if (mySelection.hasPartBeforePage(page)) { return null; }
			final ZLTextElementArea selectionStartArea = mySelection.getStartArea(page);
			if (selectionStartArea != null) { return new ZLTextSelection.Point(selectionStartArea.XStart, selectionStartArea.YEnd); }
		}
		else
		{
			if (mySelection.hasPartAfterPage(page)) { return null; }
			final ZLTextElementArea selectionEndArea = mySelection.getEndArea(page);
			if (selectionEndArea != null) { return new ZLTextSelection.Point(selectionEndArea.XEnd, selectionEndArea.YEnd); }
		}
		return null;
	}
	...
	{
		if (cursorPoint == null) { return Integer.MAX_VALUE; }
	...
	{
		if (mySelection.isEmpty()) { return ZLTextSelectionCursor.None; }
	...
	@Override
	public synchronized void preparePage(ZLPaintContext context, ZLView.PageIndex pageIndex)
	{
		setContext(context);
	...
	@Override
	public synchronized void paint(ZLPaintContext context, ZLView.PageIndex pageIndex)
	{
		setContext(context);
		final ZLFile wallpaper = getWallpaperFile();
		if (wallpaper != null) context.clear(wallpaper, getWallpaperMode());
		else context.clear(getBackgroundColor());
		if (myModel == null || myModel.getParagraphsNumber() == 0) { return; }
	...
	//
	// // TODO draw foot
	// private void drawFoot(ZLPaintContext context, int y)
	// {
	//
	...
	// final int delta = 1;
	// // context.setFont(FBView.getReader().FooterOptions.Font.getValue(),
	// // height <= 10 ? height + 3 : height + 1, height > 10, false, false,
	// // false);
	// context.setFootFont();
	//
	// final PagePosition pagePosition = this.pagePosition();
	//
	...
	// context.drawString(right - (context.getStringWidth(infoString)), height -
	// delta, info.toString());
	...
	{
		if (myModel == null || myModel.getParagraphsNumber() == 0) { return 1; }
		return myModel.getTextLength(myModel.getParagraphsNumber() - 1);
	}

	private final synchronized int getCurrentCharNumber(ZLView.PageIndex pageIndex, boolean startNotEndOfPage)
	{
		if (myModel == null || myModel.getParagraphsNumber() == 0) { return 0; }
	...
	private int sizeOfTextBeforeCursor(ZLTextWordCursor wordCursor)
	{
		...
		if (paragraphCursor == null) { return -1; }
	...
	private synchronized float computeCharsPerPage()
	{
		...
		final int effectiveHeight = (int) (textHeight - (getTextStyle().getSpaceBefore() + getTextStyle().getSpaceAfter())
				/ charsPerParagraph);
	...
	{
		int current = computeTextPageNumber(getCurrentCharNumber(ZLView.PageIndex.current, false));
		int total = computeTextPageNumber(sizeOfFullText());

		if (total > 3) { return new PagePosition(current, total); }

		preparePaintInfo(myCurrentPage);
		ZLTextWordCursor cursor = myCurrentPage.StartCursor;
		if (cursor == null || cursor.isNull()) { return new PagePosition(current, total); }
	...
	public void gotoHome()
	{
		...
		if (!cursor.isNull() && cursor.isStartOfParagraph() && cursor.getParagraphIndex() == 0) { return; }
		gotoPosition(0, 0, 0);
		preparePaintInfo();
	}
	...
	private void drawTextLine(ZLTextPage page, ZLTextLineInfo info, int from, int to)
	{
		...
		if (index != to)
		{
			...
			drawWord(area.XStart, area.YEnd - context.getDescent() - getTextStyle().getVerticalShift(), word, start, len,
					area.AddHyphenationSign, mySelection.isAreaSelected(area) ? getSelectionForegroundColor()
	...
	private ZLTextLineInfo processTextLine(ZLTextPage page, ZLTextParagraphCursor paragraphCursor, final int startIndex,
	...
	private void prepareTextLine(ZLTextPage page, ZLTextLineInfo info, int x, int y)
	{
		...
		if (!endOfParagraph)
		{
			...
			if (len > 0)
			{
				...
				page.TextElementMap.add(new ZLTextElementArea(paragraphIndex, wordIndex, 0, len, false, // is
																										// last
																										// in
																										// element
	...
	public final synchronized void gotoPosition(int paragraphIndex, int wordIndex, int charIndex)
	{
		...
		if (myModel != null && myModel.getParagraphsNumber() > 0)
		{
			...
			myNextPage.reset();
			preparePaintInfo(myCurrentPage);
			if (myCurrentPage.isEmptyPage()) scrollPage(true, ScrollingMode.NO_OVERLAPPING, 0);
		}
	}
		...
		case PaintStateEnum.TO_SCROLL_BACKWARD:
			if (!page.StartCursor.isStartOfText())
			{
				...
				case ScrollingMode.SCROLL_LINES:
					...
					break;
				case ScrollingMode.SCROLL_PERCENTAGE:
					page.StartCursor.setCursor(findStart(page, page.StartCursor, SizeUnit.PIXEL_UNIT, page.getTextHeight()
							* myOverlappingValue / 100));
					break;
				}
				buildInfos(page, page.StartCursor, page.EndCursor);
				if (page.isEmptyPage())
	...
	private int paragraphSize(ZLTextPage page, ZLTextWordCursor cursor, boolean beforeCurrentPosition, int unit)
	{
		...
		if (paragraphCursor == null) { return 0; }
	...
	{
		final ZLTextParagraphCursor paragraphCursor = cursor.getParagraphCursor();
		if (paragraphCursor == null) { return; }
	...
	{
		y -= ZLTextSelectionCursor.getHeight() / 2 + ZLTextSelectionCursor.getAccent() / 2;
		if (!mySelection.start(x, y)) { return false; }
		Application.getViewWidget().reset();
		Application.getViewWidget().repaint();
		return true;
	}
	...
	{
		if (mySelection.isEmpty()) { return 0; }
		final ZLTextElementArea selectionStartArea = mySelection.getStartArea(myCurrentPage);
		if (selectionStartArea != null) { return selectionStartArea.YStart; }
	...
	public int getSelectionEndY()
	{
		...
		if (selectionEndArea != null) { return selectionEndArea.YEnd; }
